...
class ResNet(nn.Module):
    ...
    def resnet50(pretrained_path=None, **kwargs):
        return ResNet(Bottleneck, [3, 4, 6, 3], pretrained_path, **kwargs)
...
class Pspnet(nn.Module):
    def __init__(self, num_classes, pretrained_path=None):
        ...
        self.num_classes = num_classes
        self.backbone = ResNet._resnet(Bottleneck, [3, 4, 6, 3], pretrained_path=None)
    ...
    def forward(self, x):
        x = self.backbone(x)
        x = self.decoder(x)
        x = nn.functional.interpolate(x, size=(x.size(2) * 8, x.size(3) * 8), mode='bilinear', align_corners=True)
        x = self.cls_seg(x)
        x = torch.argmax(x,dim=1,keepdim=True).type(torch.float)
        return x

if __name__ == '__main__':
    model = Pspnet(num_classes=2)
    model = model.cuda()
    a = torch.ones([3, 3, 448, 448])
    a = a.cuda()
    print(model(a).shape)